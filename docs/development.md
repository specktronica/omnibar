...
Tests are pure-logic only. There are no live Accessibility, hotkey, or event-monitor tests; tiling is covered through `TilingGeometryTests` (usable area, tile frames, keyboard cycle, coordinate conversion, Carbon flags) and the settings-decode tests. The Control+Option Start Menu chord is covered by `StartMenuShortcutTests`.
